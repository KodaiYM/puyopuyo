...
bool CPlayer::CFall::movable(Puyo::Direction direction, bool axis) const {
	return axis ? movable(mCurrentPuyoSet->point, direction) : movable(getPoint(mCurrentPuyoSet->point, mCurrentPuyoSet->angle), direction);
}
bool CPlayer::CFall::movable(const Point& point, const Puyo::Direction direction) const {
	Point p;
	p.x = point.x;
	p.y = point.y;
