...
/* ゲームクラスに対する定義 */
CGame::CGame(std::shared_ptr<ISceneChanger> changer) : CBasePhase(std::move(changer)) {
	mScene = Scene::init;
}
void CGame::Run() {
	switch (mScene) {
	case Scene::init:
	{
		const std::unordered_map<PointName, const Point> point = {
		...
		mResourceMgr = std::make_unique<CResourceMgr>();
		mResourceMgr->LoadGraphic(GraphicPath);
		mResourceMgr->LoadSound(SoundPath);

		mPlayer = std::make_unique<CPlayer>(4, 12, 6, 38, std::move(mResourceMgr), point);
	}
	mScene = Scene::run;
	case Scene::run:
		mPlayer->Run();
...
void Puyo::Draw(std::shared_ptr<const CResourceMgr> const resourceMgr) {
